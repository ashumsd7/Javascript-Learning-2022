...
const outputArray = [];
splitArray.map((item) => {
  let packA = item.slice(0, item.length / 2);
  let packB = item.slice(item.length / 2);
  packA = [...new Set(packA)].join("");
  packB = [...new Set(packB)].join("");
  packA.split("").map((char) => {
    if (packB.split("").includes(char)) outputArray.push(char);
  });
});
outputArray.sort((a, b) => a - b);
let sum = 0;
outputArray.map((item) => {
  if (item == item.toUpperCase()) sum = sum + item.charCodeAt(0) - 39 + 1;
  else sum = sum + item.charCodeAt(0) - 97 + 1;
});
